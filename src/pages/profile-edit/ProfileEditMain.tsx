...
export default function ProfileEditMain() {
  ...
  const handleDeactivate = async () => {
    if (!window.confirm("정말로 탈퇴하시겠습니까? 탈퇴 후에는 데이터를 복구할 수 없습니다.")) {
      return;
    }
        ...
        <div className="pb-6">
            ...
            <div className="flex flex-col items-center">
                ...
                <div className="mb-6 w-[111px] h-[111px] rounded-full p-[2px] bg-gradient-to-tr from-[#FFBD66] via-[#FF3D77] to-[#FF3D77]">
                    <img className="w-full h-full rounded-full object-cover" src={user?.profileImageUrl}/>
                </div>
                {/* 닉네임 */}
                <h3 className="mb-[10px] text-[20px] font-semibold leading-[1.2]">{user?.nickname} {user?.age}</h3>
                {/* 프로필 수정 버튼 */}
                <button onClick={()=>navigate("edit")} className="flex items-center px-[12px] py-[7px] rounded-full border border-gray-300 bg-gray-100"> 
                    <img src={editpen_btn} />
                    <span className="text-gray-700 text-[14px] font-medium leading-[1.2]">프로필 수정</span>
                </button>
            </div>
        </div>
        {/* 하단 화면 */}
        <div className="pt-6 flex flex-1 flex-col items-center bg-gray-100">
            ...
            >
                <div className="flex items-center gap-3 text-[#475467]">
                    ...
                    <span className="text-[16px] font-medium leading-[24px]">로그아웃</span>
                </div>
